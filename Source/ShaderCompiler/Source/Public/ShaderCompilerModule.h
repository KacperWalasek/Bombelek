...
struct FShaderCompilerParameters
{
    ...
    FShaderCompilerParameters()	{ }
    FShaderCompilerParameters(UTextureRenderTarget2D* InRenderTarget)
 ...
 */
class SHADERCOMPILER_API FShaderCompilerModule : public IModuleInterface
{
public:
    static inline FShaderCompilerModule& Get()
    {
        return FModuleManager::LoadModuleChecked<FShaderCompilerModule>("ShaderCompiler");
    }

    static inline bool IsAvailable()
    {
        return FModuleManager::Get().IsModuleLoaded("ShaderCompiler");
    }
...
public:
    ...
    void UpdateParameters(FShaderCompilerParameters& DrawParameters);

private:
    TRefCountPtr<IPooledRenderTarget> ComputeShaderOutput;
    FShaderCompilerParameters CachedShaderUsageExampleParameters;
    ...
    void Draw_RenderThread(const FShaderCompilerParameters& DrawParameters);
};
